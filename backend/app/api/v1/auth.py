...
async def register(payload: ManagerRegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    The manager cannot log in until their email has been verified.
    """
    manager, otp_code = await register_manager(db, payload)

    # Envoi de l'OTP (non bloquant en dev si SMTP non configuré)
    send_otp_email(
...
@router.post(
    ...
    summary="Verify email OTP code (Step 3)",
)
...
# ─────────────────────────────────────────────
# Téléversement de documents (étape 4)
# ─────────────────────────────────────────────
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Upload organization and manager verification documents (Step 4)",
)
async def upload_documents(
    org_id: UUID,
    company_certificate: UploadFile | None = File(default=None),
    responsible_id: UploadFile | None = File(default=None),
    db: AsyncSession = Depends(get_db),
):
    """
    Upload optional supporting verification documents (PDF, JPG, PNG up to 10 MB each).
    Step 4 of the manager self-registration process.
    """
    saved_docs = []

    if company_certificate is not None and company_certificate.filename:
        doc = await save_organization_document(
            db=db,
            organization_id=org_id,
            document_type="COMPANY_CERTIFICATE",
            file=company_certificate,
        )
        saved_docs.append(
            OrganizationDocumentOut(
                id=doc.id,
    ...
    if responsible_id is not None and responsible_id.filename:
        doc = await save_organization_document(
            db=db,
            organization_id=org_id,
